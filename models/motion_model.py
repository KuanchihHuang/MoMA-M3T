...
from .motion_encoder import TemporalEncoder, StateEncoder, SpatialEncoder
...
class MotionTrans(nn.Module):
    ...
    def forward(self, pre_coord_all, cur_coord_all, fr_diff, pre_mask=None, cur_mask=None):
        ...
        if type(pre_mask) == type(None):
            ...
            cur_mask = torch.ones(cur_coord_all.shape[0], cur_coord_all.shape[1]).cuda()
        ...
        mask = torch.zeros(b*np, l).bool().cuda()
