...
from .motion_model import MotionTrans
from .motion_tracklet import MotionTracklet
from utils.bbox_overlaps import bbox_overlaps

class MotionTracker(object):

    def __init__(self, init_track_id, is_kitti):
        ...
        if is_kitti:
            model_ckpt_name = './checkpoints/kitti.pth'
        else:
            model_ckpt_name = './checkpoints/nuscenes.pth'
    ...
    def match(self,
        ...
        if bboxes.size(0) > 0 and not self.empty:
            ...
            for ind, memo_tracker in enumerate(memo_trackers):
                ...
                fr_idx_history = torch.from_numpy(memo_tracker.get_fr_idx()).cuda()
